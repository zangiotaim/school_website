...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if (isset($_POST['newImagesToAlbum'])) {
        ...
        for ($i = 0; $i < $countfiles; $i++) {
            ...
            $storedPath = store_uploaded_file($fileUploadName, $fileUploadTmp, '../assets/images/gallery', 'assets/images/gallery', 'gallery_image');
            ...
            if ($storedPath !== null) {
                ...
                $sql = "INSERT INTO gallery_images (album_id, image_url) VALUES (?, ?)";

                if (!db_execute($connection, $sql, [$albumId, $sqlfileurl])) {
                    echo "Error: Failed to save gallery image.";
                }
            } else {
                if (has_upload_error()) {
                    echo '<script>alert(' . json_encode(get_upload_error_message()) . '); window.location.replace("add_gallery.php");</script>';
                    exit;
                }
            }
        }
    ...
    if (isset($_POST['deleteAlbum'])) {
        ...
        db_execute($connection, "DELETE FROM `gallery_album` WHERE id = ?", [$album_Id]);
        echo '
            <script>
            window.location.replace("add_gallery.php");            
            </script>';
    ...
    if (isset($_POST['deleteImage'])) {
        ...
        db_execute($connection, "DELETE FROM `gallery_images` WHERE id = ?", [$imageId]);
        echo '
            <script>
            window.location.replace("add_gallery.php");            
            </script>';
    ...
    if (isset($_POST["submit_new_album"])) {
        ...
        if (db_execute($connection, $sql, [$album_name])) {
            echo '
            <script>
            ...
            echo "Error: Failed to create album.";
        }
    }
}
?>
...
<body>
    ...
    <div class="mx-10 mb-10" id="accordion-color" data-accordion="collapse"
        ...
        <?php
        $albums = get_gallery_album_rows($connection);
        ...
        <div class="py-4 px-2 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-6">
            ...
            ';
                ...
                $result_images = get_gallery_image_rows($connection, (int) $albumId);
                $total_images = count($result_images);

                if ($total_images > 0) {
                    foreach ($result_images as $imagerow) {
                        $imageId = $imagerow["id"];
                        echo '
