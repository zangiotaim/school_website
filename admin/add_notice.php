...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['publish_notice'])) {
        ...
        $sqlfileurl = "";
        $storedPath = store_uploaded_file($fileUploadName, $fileUploadTmp, '../assets/images/notice_files', 'assets/images/notice_files', 'notice');
        ...
        $posted_by_id = get_current_admin_id($connection, $_SESSION["identity_code"]);
        $logo = $_SESSION["profile_pic"];
        $published_at = date("Y-m-d H:i:s");

        // Insert data into the school_notice table
        $sql = "INSERT INTO school_notice (logo, notice_description, posted_by_id, published_at, image_url, about) VALUES (?, ?, ?, ?, ?, ?)";

        if (db_execute($connection, $sql, [$logo, $description, $posted_by_id, $published_at, $sqlfileurl, $about_notice])) {
            echo '
            <script>
            ...
            echo "Error: Failed to publish notice.";
        }
    }

    if (isset($_POST['notice_delete'])) {
        $noticeId = $_POST["notice_id"];
        db_execute($connection, "DELETE FROM `school_notice` WHERE id = ?", [$noticeId]);
        echo '
            <script>
            window.location.replace("add_notice.php");
    ...
    if (isset($_POST['update_notice'])) {
        ...
        // Insert data into the school_notice table
        $sql = "UPDATE school_notice SET notice_description=?, about=?, image_url=?, updated_at=? WHERE id=?";

        if (db_execute($connection, $sql, [$description, $about_notice, $sqlfileurl, $updated_at, $noticeId])) {
            echo '
            <script>
            ...
            echo "Error: Failed to update notice.";
        }
    }
}
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                ...
                                $notices = get_notice_rows($connection, true);
                                $totalNotice = count($notices);
                                    ...
                                    foreach ($notices as $row) {
                                        ...
                                        echo '
                                            <tr class="border-b dark:border-gray-700">
                                                ...
                                                <td class="px-4 py-3">' . $row['display_published_at'] . '</td>
