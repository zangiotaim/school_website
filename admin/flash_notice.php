...
try {
    $flash_notice = get_flash_notice($connection);
} catch (Exception $e) {
...
if (isset($_POST['update_flash_notice'])) {
    ...
    $storedPath = store_uploaded_file($fileUploadName, $fileUploadTmp, '../assets/images/flash_notice', 'assets/images/flash_notice', 'flash_notice');
    ...
    $sql = "UPDATE flash_notice SET title=?, message=?, is_enabled=?, image_url=? WHERE id=1";

    if (db_execute($connection, $sql, [$flash_title, $flash_message, $is_enabled, $sqlfileurl])) {
        echo '
        <script>
        ...
        echo "Error: Failed to update flash notice.";
    }
    }
}
?>
                            ...
                            <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2">
                                <div>
                                    ...
                                    <select name="is_enabled"
                                        ...
                                        <option <?php if ((int) $flash_notice['is_enabled'] === 1) {
                                            echo 'selected';
                                        } ?>
                                            value="1">ON</option>
                                        <option <?php if ((int) $flash_notice['is_enabled'] === 0) {
                                            echo 'selected';
                                        } ?>
                                            value="0">OFF</option>
