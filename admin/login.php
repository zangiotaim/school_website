...
if (isset($_POST['submit'])) {
    ...
    $row = db_select_one($connection, "SELECT * FROM admins WHERE identity_code = ? LIMIT 1", [$identity_code]);

    if ($row && password_verify($password, $row["password"])) {
        session_regenerate_id(true);
        $_SESSION["admin_id"] = (int) $row["id"];
        $_SESSION["admin_role"] = strtolower((string) ($row["role"] ?? 'scribe'));
        ...
        <section class="bg-gray-50 dark:bg-gray-900">
            <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
                <a href="#" class="flex items-center mb-6 text-2xl font-semibold text-gray-900 dark:text-white">
                    <img class="w-80 h-22 mr-2" src="../assets/images/defaults/header_logo.png" alt="logo">
