...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_scribe'])) {
        ...
        $storedPath = store_uploaded_file($fileUploadName, $fileUploadTmp, '../assets/images/admin', 'assets/images/admin', 'admin_user');
        ...
        $scribeUsername = $_POST['scribeUsername'];
        $scribeIdentity = $_POST['scribeIdentity'];
        $passwordHash = password_hash($_POST['password'], PASSWORD_DEFAULT);
        $updatedAt = date("Y-m-d H:i:s");

        // Insert data into the school_notice table
        $sql = "UPDATE admins SET username=?, identity_code=?, password=?, image=?, updated_at=? WHERE id=?";

        if (db_execute($connection, $sql, [$scribeUsername, $scribeIdentity, $passwordHash, $sqlfileurl, $updatedAt, $scribeId])) {
            echo '
            <script>
            ...
            echo "Error: Failed to update scribe.";
        }
    }
    ...
    if (isset($_POST['update_admin'])) {
        ...
        $currentAdminId = admin_get_session_admin_id();
        ...
        $currentPasswordHash = get_admin_password_hash($connection, $_SESSION["identity_code"]);

        if ($currentPasswordHash && password_verify($previousPassword, $currentPasswordHash)) {
            // Update data into the admin table
            $sql = "UPDATE admins SET identity_code=?, password=?, image=?, updated_at=? WHERE id=?";

            if ($currentAdminId && db_execute($connection, $sql, [$newIdentity, $passwordHash, $sqlfileurl, date("Y-m-d H:i:s"), $currentAdminId])) {
                $_SESSION["adminPassAttempt"] = 3;
                $_SESSION["identity_code"] = $newIdentity;
                $_SESSION["profile_pic"] = $sqlfileurl;
                echo '
    <script>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                ...
                                $Scribes = get_admin_rows($connection);
                                $totalScribes = count($Scribes);

                                if ($totalScribes > 0) {
                                    foreach ($Scribes as $row) {
                                        $ScribesId = $row['id'];
                                        echo '
                                    ...
                                    <form method="post" onsubmit="return validatePassword'.$row['id'].'()" id="UpdateUsers' . $ScribesId . '" enctype="multipart/form-data">
                                        ...
                                        <div><label for="new_file" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">📁 File (None will be previous)</label></div><br>
                                            ...
                                            <div class="sm:col-span-2">
                                                <label for="scribeUsername" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">🧑🏻 Username</label>
                                                <input type="text" name="scribeUsername" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" value="' . $row['username'] . '" placeholder="bisswass_niroula">
