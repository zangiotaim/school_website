...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_committe'])) {
        ...
        $sqlfileurl = "";
        $storedPath = store_uploaded_file($fileUploadName, $fileUploadTmp, '../assets/images/staff', 'assets/images/staff', 'committee_member');
        ...
        if (db_execute($connection, $sql, [$committeName, $committePosition, $committePhone, $sqlfileurl])) {
            ...
            echo "Error: Failed to add committee member.";
        }
    }
    if (isset($_POST['add_staff'])) {
        ...
        $sqlfileurl = "";
        $storedPath = store_uploaded_file($fileUploadName, $fileUploadTmp, '../assets/images/staff', 'assets/images/staff', 'teacher');
        if ($fileUploadName !== '' && $storedPath === null && has_upload_error()) {
            echo '<script>alert(' . json_encode(get_upload_error_message()) . ');</script>';
        } elseif ($storedPath !== null) {
            $sqlfileurl = $storedPath;
        ...
        // Insert data into the school_notice table
        $sql = "INSERT INTO staffs (name, post, subject, contact, qualification, image_src) VALUES (?, ?, ?, ?, ?, ?)";
    ...
    if (isset($_POST['committe_delete'])) {
        ...
        db_execute($connection, "DELETE FROM `management_committee` WHERE id = ?", [$committeId]);
        echo '
            <script>
            window.location.replace("staff.php");            
            </script>';
    ...
    if (isset($_POST['staffDelete'])) {
        ...
        db_execute($connection, "DELETE FROM `staffs` WHERE id = ?", [$staffId]);
        echo '
            <script>
            window.location.replace("staff.php");            
            </script>';
    ...
    if (isset($_POST['update_committe'])) {
        ...
        if (db_execute($connection, $sql, [$committeName, $committePost, $committePhone, $sqlfileurl, $committeId])) {
            ...
            echo "Error: Failed to update committee member.";
        }
    }
    ...
    if (isset($_POST['update_staffs'])) {
        ...
        // Insert data into the school_notice table
        $sql = "UPDATE staffs SET name=?, post=?, subject=?, qualification=?, contact=?, image_src=? WHERE id=?";
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                ...
                                $managementCommitte = get_management_committee_rows($connection);
                                $totalmanagementCommitte = count($managementCommitte);
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody>
                                ...
                                $managementCommitte = get_staff_rows($connection);
                                $totalmanagementCommitte = count($managementCommitte);
