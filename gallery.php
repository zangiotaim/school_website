...
if (!isset($_SESSION["selectedAlbum"]) && !isset($_SESSION["selectedAlbumId"])) {
    header("Location: gallery_albums.php");
    exit();
}
...
<div class="mb-10 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4">
    <?php
        $selectedAlbum = $_SESSION["selectedAlbum"] ?? 'Album';
        $selectedAlbumId = isset($_SESSION["selectedAlbumId"]) ? (int) $_SESSION["selectedAlbumId"] : 0;

        $photos = db_select_all($connection, "SELECT * FROM `gallery_images` WHERE `album_id` = ? ORDER BY id DESC;", [$selectedAlbumId]);

        $total12photos = count($photos);
