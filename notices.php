...
<body>
    ...
    $notices = get_notice_rows($connection);
                            ...
                            <div class="py-2 flex flex-row items-center justify-between">
                                <div class="flex flex-row items-center">
                                    <a class="flex flex-row items-center focus:outline-none focus:shadow-outline rounded-lg">
                                        ...
                                        <p class="ml-2 text-base font-medium text-sm md:text-base">' . $row['display_posted_by'] . '</p>
                                    </a>
                                </div>
                                <div class="flex flex-row items-center">
                                    <p class="text-xs font-semibold text-gray-500">' . $row['display_published_at'] . '</p>
                                </div>
                            </div>
                            <div class="mt-2">
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['action']) && isset($_POST['notice_id'])) {
        ...
        if ($action == 'view') {
            db_execute($connection, "UPDATE `school_notice` SET total_views = total_views + 1 WHERE id = ?", [$notice_id]);
        } elseif ($action == 'download') {
            db_execute($connection, "UPDATE `school_notice` SET total_downloads = total_downloads + 1 WHERE id = ?", [$notice_id]);
        }
        exit;
    }
}
